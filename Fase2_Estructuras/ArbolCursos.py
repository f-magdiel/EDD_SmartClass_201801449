...
class Arbolb:
    ...
    def buscarAgregar(self, _codigo, _nombre, _noCreditos, _codPrerrequisito, _obligatorio):
        try:
            codigo = int(_codigo)
            aux = self.buscar(codigo, self.raiz)
            if (aux != None):
                print("Ya existe en arbol")
                return aux

        except AttributeError:
            self.insertar(_codigo, _nombre, _noCreditos, _codPrerrequisito, _obligatorio)
            print("Insertado en arbol cursos student")
    ...
    def dividirRama(self, rama):
        ...
        for i in range(self.orden + 1):
            i += 1
            if (i < self.orden + 1):
                nodo = Nodo(actual.codigo, actual.nombre, actual.noCreditos, actual.codPrerrequisito,
                            actual.obligatorio)
